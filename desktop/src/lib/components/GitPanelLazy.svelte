<script lang="ts">
  ...
  async function loadGitStatus() {
    ...
    try {
      if (tauriInvoke) {
        gitStatus = await tauriInvoke('git_status');
      } else {
  ...
  async function loadDiff(file: GitFile) {
    ...
    try {
      if (tauriInvoke) {
        diffContent = await tauriInvoke('git_diff', { path: file.path, staged: file.staged });
      } else {
...
+    <button on:click={handleClick}>Click me</button>
  ...
  async function stageFile(file: GitFile) {
    try {
      if (tauriInvoke) {
        await tauriInvoke('git_stage', { path: file.path });
      }
      await loadGitStatus();
    } catch (error) {
  ...
  async function unstageFile(file: GitFile) {
    try {
      if (tauriInvoke) {
        await tauriInvoke('git_unstage', { path: file.path });
      }
      await loadGitStatus();
    } catch (error) {
  ...
  async function stageAll() {
    try {
      if (tauriInvoke) {
        await tauriInvoke('git_stage_all');
      }
      await loadGitStatus();
    } catch (error) {
  ...
  async function unstageAll() {
    try {
      if (tauriInvoke) {
        await tauriInvoke('git_unstage_all');
      }
      await loadGitStatus();
    } catch (error) {
  ...
  async function commit() {
    ...
    try {
      if (tauriInvoke) {
        await tauriInvoke('git_commit', { message: commitMessage });
      }
      commitMessage = '';
      await loadGitStatus();
    } catch (error) {
        ...
        {#if showDiffView && selectedFile}
          ...
          <div class="diff-view">
            ...
            <div class="diff-content">
              {#each parseDiff(diffContent) as line, index (index)}
        ...
        {:else}
          ...
          <div class="file-sections">
            ...
            <div class="file-section">
              ...
              {#if gitStatus?.unstaged?.length && gitStatus.unstaged.length > 0}
                <div class="file-list">
                  {#each gitStatus?.unstaged || [] as file (file)}
                    <div class="file-item">
                      <span 
                        class="file-status" 
                        style="color: {statusColors[file.status]}"
