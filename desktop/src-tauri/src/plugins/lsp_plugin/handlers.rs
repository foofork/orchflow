...
impl LspHandlers {
    ...
    async fn handle_file_opened(&mut self, path: &str) -> Result<(), String> {
        if let Some(language) = config::get_language_for_file(path, self.server_manager.get_configs()) {
            ...
            self.file_to_server.insert(path.to_string(), language.clone());
            ...
            if let Some(server) = self.server_manager.get_server(&language) {
                ...
                server.stdin_tx.send(notification_str).await
    ...
    async fn handle_completion_request(&mut self, params: Value) -> Result<Value, String> {
        ...
        self.forward_lsp_request("textDocument/completion", params).await
    }
    ...
    async fn handle_definition_request(&mut self, params: Value) -> Result<Value, String> {
        self.forward_lsp_request("textDocument/definition", params).await
    }
    
    async fn handle_references_request(&mut self, params: Value) -> Result<Value, String> {
        self.forward_lsp_request("textDocument/references", params).await
    }
    
    async fn handle_formatting_request(&mut self, params: Value) -> Result<Value, String> {
        self.forward_lsp_request("textDocument/formatting", params).await
    }
    
    async fn handle_rename_request(&mut self, params: Value) -> Result<Value, String> {
        self.forward_lsp_request("textDocument/rename", params).await
    }
    ...
    async fn handle_document_symbol_request(&mut self, params: Value) -> Result<Value, String> {
        self.forward_lsp_request("textDocument/documentSymbol", params).await
    }
    ...
    async fn forward_lsp_request(&mut self, method: &str, params: Value) -> Result<Value, String> {
        ...
        let path = LspProtocol::uri_to_path(uri)
            .ok_or("Invalid URI format")?;
        
        let language = self.file_to_server.get(&path)
            .ok_or("No language server associated with file")?;
        
        let server = self.server_manager.get_server(language)
