...
impl Plugin for TerminalPlugin {
    ...
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            ...
            description: "Advanced terminal features including history, suggestions, and search"
                .to_string(),
            capabilities: vec![
        ...
        match event {
            Event::PaneCreated { pane } => {
                if matches!(pane.pane_type, PaneType::Terminal) {
                    ...
                    self.terminals
                        .insert(pane.id.clone(), TerminalHistory::new(pane.id.clone()));
                }
            }
    ...
    async fn handle_request(&mut self, method: &str, params: Value) -> Result<Value, String> {
        match method {
            ...
            "terminal.history" => {
                let pane_id = params["pane_id"].as_str().ok_or("Missing pane_id")?;
                let limit = params["limit"].as_u64().unwrap_or(50) as usize;

                if let Some(history) = self.terminals.get(pane_id) {
                    let commands: Vec<_> =
                        history.commands.iter().rev().take(limit).cloned().collect();
            ...
            "terminal.historyCommand" => {
                let pane_id = params["pane_id"].as_str().ok_or("Missing pane_id")?;
                let index = params["index"].as_i64().unwrap_or(-1) as i32;
            ...
            "terminal.addAlias" => {
                let alias = params["alias"].as_str().ok_or("Missing alias")?;
            ...
            "terminal.setCaptureOutput" => {
                self.capture_output = params["enabled"].as_bool().unwrap_or(true);
