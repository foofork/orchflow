...
impl FileTreeCache {
    ...
    pub async fn build_tree(
        ...
        let root = self
            .create_node(root_path, true, gitignore_patterns)
            .await?;
        ...
        while let Some((dir_path, depth)) = queue.pop_front() {
            ...
            match self
                .load_children_simple(&dir_path, gitignore_patterns)
                .await
            {
                Ok(child_paths) => {
                    ...
                    for child_path in child_paths {
                        match self
                            .create_node(&child_path, false, gitignore_patterns)
                            .await
                        {
                            Ok(child_node) => {
    ...
    pub async fn expand_directory(
        ...
        for child_path in children {
            let node = self
                .create_node(&child_path, true, gitignore_patterns)
                .await?;
