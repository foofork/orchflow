#[cfg(test)]
mod tests {
    use crate::file_manager::{FileEntryType, FileManager, FileOperationResult};
    use std::path::Path;
    ...
    async fn test_list_directory() {
        ...
        assert!(entries
            .iter()
            .any(|e| e.name == "file1.txt" && e.file_type == FileEntryType::File));
        assert!(entries
            .iter()
            .any(|e| e.name == "file2.txt" && e.file_type == FileEntryType::File));
        assert!(entries
            .iter()
            .any(|e| e.name == "subdir" && e.file_type == FileEntryType::Directory));
    }
    ...
    async fn test_copy_files() {
        ...
        // Copy files
        let result = manager
            .copy_files(vec![file1.clone(), file2.clone()], &dest_dir)
            .await;
    ...
    async fn test_search() {
        ...
        // Search for .rs files
        let results = manager
            .search_files("*.rs", Some(temp_dir.path()))
            .await
            .unwrap();
