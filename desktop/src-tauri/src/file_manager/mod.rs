...
impl FileManager {
    pub fn new(root_path: PathBuf) -> Self {
        ...
        let operations = FileOperations::new(history.clone(), cache.clone(), root_path.clone());
    ...
    pub async fn build_file_tree(&self, max_depth: Option<usize>) -> Result<FileNode> {
        self.cache
            .build_tree(&self.root_path, max_depth, &self.gitignore_patterns)
            .await
    }
    ...
    pub async fn expand_directory(&self, path: &Path) -> Result<Vec<FileNode>> {
        self.cache
            .expand_directory(path, &self.gitignore_patterns)
            .await
    }
    ...
    pub async fn copy_file(&self, source: &str, destination: &str) -> Result<FileOperationResult> {
        ...
        self.operations
            .copy_files(vec![src], dst.parent().unwrap_or(&dst))
            .await?;
    ...
    pub async fn list_directory(&self, path: &str) -> Result<Vec<FileEntry>> {
        ...
        let mut dir_entries = tokio::fs::read_dir(&dir_path).await.map_err(|e| {
            crate::error::OrchflowError::FileOperationError {
                path: dir_path.clone(),
    ...
    pub fn get_all_git_statuses(
        &self,
    ) -> Result<std::collections::HashMap<PathBuf, FileGitStatus>> {
        if let Some(ref git) = self.git_integration {
