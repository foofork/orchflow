...
pub async fn get_file_preview(path: &Path, max_lines: usize, max_file_size: u64) -> Result<String> {
    ...
    if metadata.len() > max_file_size {
        return Ok(format!("File too large to preview ({} bytes)", metadata.len()));
    }
...
pub async fn search_files(pattern: &str, search_path: &Path, project_root: &Path) -> Result<Vec<PathBuf>> {
