...
pub async fn install_module(
    ...
    if let Some(existing) = loader.get_module(&name) {
        return Err(format!(
            "Module '{}' already installed (version {})",
...
pub async fn update_module_config(
    ...
    if let Some(module) = loader.get_module(&module_name) {
        ...
        state_store
            .set_setting(&format!("module.{}.config", module_name), &config_str)
            .await
            .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn validate_module_manifest(manifest: Value) -> Result<bool, String> {
...
pub async fn create_module_template(
    ...
    if !valid_types.contains(&module_type.as_str()) {
        return Err(format!(
            "Invalid module type: {}. Must be one of: {:?}",
            module_type, valid_types
        ));
    }
...
mod tests {
    ...
    fn test_validate_config_against_schema() {
        ...
        assert!(result
            .unwrap_err()
            .contains("Missing required property: enabled"));
    ...
    fn test_module_registry_search() {
        ...
        let mock_modules = vec![ModuleRegistryEntry {
