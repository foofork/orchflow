...
#[tauri::command]
pub async fn list_backend_sessions(manager: State<'_, Manager>) -> Result<Vec<BackendSession>> {
    let sessions = manager.mux_backend.list_sessions().await.map_err(|e| {
        crate::error::OrchflowError::BackendError {
    ...
    let backend_sessions: Vec<BackendSession> = sessions
        .into_iter()
...
#[tauri::command]
pub async fn attach_backend_session(manager: State<'_, Manager>, session_id: String) -> Result<()> {
    manager
        .mux_backend
...
#[tauri::command]
pub async fn detach_backend_session(manager: State<'_, Manager>, session_id: String) -> Result<()> {
    manager
        .mux_backend
...
#[tauri::command]
pub async fn kill_backend_session(manager: State<'_, Manager>, session_id: String) -> Result<()> {
    manager
        .mux_backend
...
pub async fn list_backend_panes(
    ...
    // Convert to frontend-friendly format
    let backend_panes: Vec<BackendPane> = panes
        .into_iter()
        .map(|p| BackendPane {
            id: p.id,
...
#[tauri::command]
pub async fn sync_backend_sessions(manager: State<'_, Manager>) -> Result<SyncResult> {
    // Get backend sessions
    let backend_sessions = manager.mux_backend.list_sessions().await.map_err(|e| {
        crate::error::OrchflowError::BackendError {
            operation: "list_sessions".to_string(),
            reason: e.to_string(),
        }
    ...
    for backend_session in &backend_sessions {
        if !orchflow_sessions
            .iter()
            .any(|s| s.name == backend_session.name)
        {
    ...
    for orchflow_session in &orchflow_sessions {
        if !backend_sessions
            .iter()
            .any(|s| s.name == orchflow_session.name)
        {
