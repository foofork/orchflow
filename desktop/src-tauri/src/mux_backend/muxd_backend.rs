use super::{MuxBackend, MuxError, Session, Pane, SplitType, PaneSize};
...
pub struct MuxdBackend {
    ...
    pending_requests: Arc<Mutex<HashMap<u64, tokio::sync::oneshot::Sender<Result<Value, MuxError>>>>>,
...
impl MuxdBackend {
    ...
    pub async fn new(url: String) -> Result<Self, MuxError> {
        ...
        let pending_requests: Arc<Mutex<HashMap<u64, tokio::sync::oneshot::Sender<Result<Value, MuxError>>>>> = Arc::new(Mutex::new(HashMap::new()));
    ...
    async fn request(&self, method: &str, params: Value) -> Result<Value, MuxError> {
        ...
        // Send request
        self.command_tx.send(request.to_string())
...
impl MuxBackend for MuxdBackend {
    async fn create_session(&self, name: &str) -> Result<String, MuxError> {
        let response = self.request("session.create", json!({
            "name": name
        })).await?;
    ...
    async fn capture_pane(&self, pane_id: &str) -> Result<String, MuxError> {
        let response = self.request("pane.read", json!({
            "pane_id": pane_id,
            "lines": 1000
        })).await?;
    ...
    async fn list_sessions(&self) -> Result<Vec<Session>, MuxError> {
        ...
        let sessions = response.get("sessions")
        ...
        for session_data in sessions {
            if let (Some(id), Some(name)) = (
                ...
                let session = Session {
                    ...
                    window_count: session_data.get("pane_count")
                        .and_then(|v| v.as_u64())
    ...
    async fn kill_session(&self, session_id: &str) -> Result<(), MuxError> {
        self.request("session.delete", json!({
            "session_id": session_id
        })).await?;
    ...
    async fn list_panes(&self, session_id: &str) -> Result<Vec<Pane>, MuxError> {
        let response = self.request("pane.list", json!({
            "session_id": session_id
        })).await?;
        
        let panes = response.get("panes")
            .and_then(|v| v.as_array())
            .ok_or_else(|| MuxError::ParseError("Invalid response".to_string()))?;
