...
use super::backend::{MuxBackend, MuxError, Session, Pane, SplitType, PaneSize};
use crate::tmux::{TmuxManager, TmuxSession, TmuxError as OldTmuxError};
use std::sync::Arc;
    ...
    async fn tmux_command(&self, args: Vec<&str>) -> Result<String, MuxError> {
        ...
        if !output.status.success() {
            ...
            if stderr.contains("session not found") || stderr.contains("can't find session") {
                return Err(MuxError::session_not_found(
                    stderr.trim().to_string()
                ));
            } else if stderr.contains("pane not found") || stderr.contains("can't find pane") {
                return Err(MuxError::pane_not_found(
                    stderr.trim().to_string()
                ));
            } else if stderr.contains("server not found") || stderr.contains("no server running") {
                return Err(MuxError::backend_unavailable(
                    "Tmux server is not running"
                ));
            }
            
            return Err(MuxError::command_failed(format!("Tmux command failed: {}", stderr)));
        }
...
impl MuxBackend for TmuxBackend {
    async fn create_session(&self, name: &str) -> Result<String, MuxError> {
        ...
        if name.contains(':') || name.contains('.') {
            error!("Invalid session name '{}': contains illegal characters", name);
            return Err(MuxError::invalid_state(
                format!("Session name '{}' contains illegal characters (: or .)", name)
            ));
        }
        
        info!("Creating tmux session: {}", name);
        let manager = self.tmux_manager.lock().await;
        let session = manager.create_session(name)
            .map_err(|e| {
    ...
    async fn create_pane(&self, session_id: &str, split: SplitType) -> Result<String, MuxError> {
        info!("Creating pane in session '{}' with split type: {:?}", session_id, split);
        let manager = self.tmux_manager.lock().await;
        ...
        let pane = manager.create_pane(session_id, None)
            .map_err(|e| {
    ...
    async fn send_keys(&self, pane_id: &str, keys: &str) -> Result<(), MuxError> {
        ...
        if keys.len() > 10000 {
            warn!("Sending very large key sequence ({} chars) to pane '{}'", keys.len(), pane_id);
        }
        
        debug!("Sending keys to pane '{}': {}", pane_id, 
            if keys.len() > 100 { 
        ...
        let manager = self.tmux_manager.lock().await;
        manager.send_keys(pane_id, keys)
            .map_err(|e| {
    ...
    async fn capture_pane(&self, pane_id: &str) -> Result<String, MuxError> {
        ...
        let manager = self.tmux_manager.lock().await;
        let content = manager.capture_pane(pane_id, None)
            .map_err(|e| {
    ...
    async fn list_sessions(&self) -> Result<Vec<Session>, MuxError> {
        ...
        let manager = self.tmux_manager.lock().await;
        let tmux_sessions = manager.list_sessions()
            .map_err(|e| {
                error!("Failed to list sessions: {:?}", e);
                Self::convert_error(e)
            })?;
        let session_count = tmux_sessions.len();
        let sessions: Vec<Session> = tmux_sessions.into_iter()
    ...
    async fn kill_session(&self, session_id: &str) -> Result<(), MuxError> {
        ...
        self.tmux_command(vec!["kill-session", "-t", session_id]).await
            .map_err(|e| {
    ...
    async fn resize_pane(&self, pane_id: &str, size: PaneSize) -> Result<(), MuxError> {
        if size.width == 0 || size.height == 0 {
            ...
            return Err(MuxError::invalid_state(
                format!("Invalid pane size: {}x{} (dimensions must be > 0)", size.width, size.height)
            ));
        }
        
        if size.width > 9999 || size.height > 9999 {
            warn!("Very large pane size requested: {}x{}", size.width, size.height);
        }
        
        info!("Resizing pane '{}' to {}x{}", pane_id, size.width, size.height);
        let manager = self.tmux_manager.lock().await;
        manager.resize_pane(pane_id, size.width as i32, size.height as i32)
            .map_err(|e| {
    ...
    async fn select_pane(&self, pane_id: &str) -> Result<(), MuxError> {
        ...
        self.tmux_command(vec!["select-pane", "-t", pane_id]).await
            .map_err(|e| {
    ...
    async fn attach_session(&self, session_id: &str) -> Result<(), MuxError> {
        ...
        self.tmux_command(vec!["attach-session", "-t", session_id]).await
            .map_err(|e| {
    ...
    async fn detach_session(&self, session_id: &str) -> Result<(), MuxError> {
        ...
        self.tmux_command(vec!["detach-client", "-s", session_id]).await
            .map_err(|e| {
...
mod tests {
    ...
    async fn test_create_and_list_sessions() {
        ...
        let session_id = backend.create_session("test_session").await
    ...
    async fn test_pane_lifecycle() {
        ...
        // Create session
        let session_id = backend.create_session("pane_test").await
            .expect("Should create session");
        ...
        // Send keys
        backend.send_keys(&pane_id, "echo hello").await
    ...
    async fn test_pane_resize() {
        ...
        let session_id = backend.create_session("resize_test").await
            .expect("Should create session");
    ...
    async fn test_error_handling() {
        ...
        let result = backend.create_pane("non_existent_session", SplitType::None).await;
