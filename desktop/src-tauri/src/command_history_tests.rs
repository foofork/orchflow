#[cfg(test)]
mod tests {
    use crate::command_history::{CommandEntry, CommandHistory};
    ...
    fn create_test_history() -> (CommandHistory, TempDir) {
        ...
        let store =
            Arc::new(SimpleStateStore::new_with_file(temp_dir.path().join("test.db")).unwrap());
    ...
    async fn test_search_by_command() {
        ...
        let results = history
            .search("test command", None, None, 10)
            .await
            .unwrap();
        ...
        let results = history
            .search("non-existent", None, None, 10)
            .await
            .unwrap();
    ...
    async fn test_get_suggestions() {
        ...
        for _ in 0..5 {
            history
                .add_command(create_test_entry("git status"))
                .await
                .unwrap();
        }
        for _ in 0..3 {
            history
                .add_command(create_test_entry("git add ."))
                .await
                .unwrap();
        }
        history
            .add_command(create_test_entry("git commit"))
            .await
            .unwrap();
    ...
    async fn test_get_stats() {
        ...
        for _ in 0..3 {
            history
                .add_command(create_test_entry("ls -la"))
                .await
                .unwrap();
        }
        history
            .add_command(create_test_entry("cd /home"))
            .await
            .unwrap();
    ...
    async fn test_cleanup_old_entries() {
        ...
        for i in 0..2 {
            history
                .add_command(create_test_entry(&format!("recent command {}", i)))
                .await
                .unwrap();
        }
    ...
    async fn test_search_with_filters() {
        ...
        // Search by both
        let results = history
            .search("", Some("pane1"), Some("session1"), 10)
            .await
            .unwrap();
        assert_eq!(results.len(), 1);
    ...
    async fn test_cache_overflow() {
        ...
        for i in 0..1100 {
            history
                .add_command(create_test_entry(&format!("command {}", i)))
                .await
                .unwrap();
        }
