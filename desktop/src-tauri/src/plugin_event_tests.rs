...
mod plugin_event_tests {
    ...
    async fn test_plugin_receives_events() {
        ...
        plugin.handle_event(&Event::FileOpened {
    ...
    async fn test_custom_event_handling() {
        ...
        // Test custom event
        plugin.handle_event(&Event::Custom {
    ...
    async fn test_file_watch_events() {
        ...
        plugin.handle_event(&Event::FileWatchStarted {
            path: "/test/watch".to_string(),
            recursive: true,
        }).await.unwrap();
        
        plugin.handle_event(&Event::FileWatchEvent {
            event: crate::file_watcher::FileWatchEvent::Modified("/test/watch/file.txt".into()),
        }).await.unwrap();
        
        plugin.handle_event(&Event::FileWatchStopped {
            path: "/test/watch".to_string(),
        }).await.unwrap();
    ...
    async fn test_command_events() {
        ...
        // Test command events
        plugin.handle_event(&Event::CommandExecuted {
            pane_id: "test-pane".to_string(),
            command: "ls -la".to_string(),
        }).await.unwrap();
        
        plugin.handle_event(&Event::CommandCompleted {
            pane_id: "test-pane".to_string(),
            exit_code: 0,
        }).await.unwrap();
