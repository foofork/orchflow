...
use super::types::{PaneState, SessionState};
...
impl PersistenceManager {
    ...
    pub async fn save_session(&self, session: &SessionState) -> Result<()> {
        ...
        self.store
            .set(&format!("session:{}", session.id), &session_data)
            .await
    ...
    pub async fn load_session(&self, session_id: &str) -> Result<Option<SessionState>> {
        ...
        match session_data {
            Some(data) => {
                let session: SessionState = serde_json::from_str(&data).map_err(|e| {
                    OrchflowError::internal_error("deserialize_session", &e.to_string())
                })?;
                Ok(Some(session))
            }
            None => Ok(None),
    ...
    pub async fn save_pane(&self, pane: &PaneState) -> Result<()> {
        ...
        self.store
            .set(&format!("pane:{}", pane.id), &pane_data)
            .await
    ...
    pub async fn load_pane(&self, pane_id: &str) -> Result<Option<PaneState>> {
        let pane_data = self
            .store
            .get(&format!("pane:{}", pane_id))
            .await
            .map_err(|e| OrchflowError::internal_error("load_pane", &e.to_string()))?;

        match pane_data {
            Some(data) => {
                let pane: PaneState = serde_json::from_str(&data).map_err(|e| {
                    OrchflowError::internal_error("deserialize_pane", &e.to_string())
                })?;
                Ok(Some(pane))
            }
            None => Ok(None),
