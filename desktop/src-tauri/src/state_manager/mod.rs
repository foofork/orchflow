...
impl StateManager {
    ...
    pub async fn create_pane(
        ...
        // Create the pane
        let pane = self.panes.create_pane(session_id.clone(), pane_type, backend_id).await?;
        
        // Add pane to session
        self.sessions.add_pane_to_session(&session_id, &pane.id).await?;
    ...
    pub async fn delete_pane(&self, pane_id: &str) -> Result<()> {
        ...
        let pane = self.panes.get_pane(pane_id).await
            .ok_or_else(|| OrchflowError::pane_not_found(pane_id))?;
        
        // Remove pane from session
        self.sessions.remove_pane_from_session(&pane.session_id, pane_id).await?;
    ...
    pub async fn create_session_with_pane(
        ...
        // Create pane
        let pane = self.create_pane(session.id.clone(), pane_type, backend_id).await?;
        ...
        // Get updated session
        let updated_session = self.get_session(&session.id).await
