...
impl PaneManager {
    ...
    pub async fn create_pane(
        ...
        // Emit event
        self.events.emit(StateEvent::PaneCreated { 
            pane: pane.clone() 
        });
    ...
    pub async fn update_pane(&self, pane: PaneState) -> Result<()> {
        ...
        // Emit event
        self.events.emit(StateEvent::PaneUpdated { 
            pane: pane.clone() 
        });
    ...
    pub async fn update_pane_title(&self, pane_id: &str, title: String) -> Result<()> {
        let mut pane = self.get_pane(pane_id).await
    ...
    pub async fn update_pane_working_dir(&self, pane_id: &str, working_dir: Option<String>) -> Result<()> {
        let mut pane = self.get_pane(pane_id).await
            .ok_or_else(|| OrchflowError::pane_not_found(pane_id))?;
    ...
    async fn validate_pane(&self, pane: &PaneState) -> Result<()> {
        if pane.id.is_empty() {
            return Err(OrchflowError::validation_error("pane_id", "Pane ID cannot be empty"));
        }
        
        if pane.session_id.is_empty() {
            return Err(OrchflowError::validation_error("session_id", "Session ID cannot be empty"));
        }
        
        if pane.title.is_empty() {
            return Err(OrchflowError::validation_error("pane_title", "Pane title cannot be empty"));
        }
