...
mod tests {
    ...
    async fn test_pane_management() {
        ...
        let session = manager
            .create_session("Pane Test".to_string())
            .await
            .unwrap();
    ...
    async fn test_shell_type_detection() {
        ...
        let has_common_shell = common_shells
            .iter()
            .any(|&shell| available_names.iter().any(|name| name == shell));
    ...
    async fn test_session_with_multiple_panes() {
        ...
        // Create session
        let session = manager
            .create_session("Multi-pane Session".to_string())
            .await
            .unwrap();

        // Create multiple panes
        let pane_ids: Vec<String> = (0..5)
            .map(|i| {
                let manager = manager.clone();
    ...
    async fn test_session_persistence() {
        ...
        let store =
            Arc::new(crate::simple_state_store::SimpleStateStore::new_with_file(&temp_db).unwrap());
        let manager = Arc::new(StateManager::new(store));
    ...
    async fn test_pane_content_management() {
        ...
        let session = manager
            .create_session("Content Test".to_string())
            .await
            .unwrap();
        let pane = manager
            .create_pane(session.id.clone(), "editor".to_string(), None, None)
            .await
            .unwrap();
        ...
        manager
            .update_pane_content(&pane.id, content)
            .await
            .unwrap();
