...
#[tauri::command]
pub async fn get_layout(session_id: String, state: State<'_, AppState>) -> Result<GridLayout, String> {
    let layouts = state.layouts.lock().unwrap();
    layouts.get(&session_id)
        .cloned()
...
pub async fn close_layout_pane(
    ...
    let backend_pane_id = {
        let layouts = state.layouts.lock().unwrap();
        let layout = layouts.get(&session_id)
            .ok_or_else(|| "Layout not found".to_string())?;
...
pub async fn resize_layout_pane(
    ...
    let (backend_pane_id, width, height) = {
        ...
        layout.panes.get(&pane_id)
            .and_then(|pane| pane.pane_id.as_ref().map(|id| (
                id.clone(),
