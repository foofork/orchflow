...
impl GridLayout {
    ...
    pub fn split_pane(&mut self, pane_id: &str, split_type: SplitType, percent: u8) -> Result<(String, String)> {
    ...
    fn recalculate_bounds(&mut self, pane_id: &str) -> Result<()> {
        ...
        let split_type = pane.split_type.as_ref()
    ...
    pub fn close_pane(&mut self, pane_id: &str) -> Result<()> {
        ...
        let parent = self.panes.get(&parent_id)
            .ok_or_else(|| OrchflowError::pane_not_found(&parent_id))?
            .clone();
        
        // Find sibling
        let sibling_id = parent.children.iter()
            .find(|&id| id != pane_id)
            .ok_or_else(|| OrchflowError::LayoutError {
        ...
        let sibling = self.panes.get(&sibling_id)
