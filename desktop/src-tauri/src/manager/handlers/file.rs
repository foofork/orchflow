...
pub async fn create_file(
    ...
    use std::path::Path;
    file_manager
        .create_file(Path::new(path), content.unwrap_or(""))
        .await
...
pub async fn delete_path(manager: &Manager, path: &str, permanent: bool) -> Result<Value, String> {
    let file_manager = manager
        .file_manager
        .as_ref()
        .ok_or_else(|| "File manager not initialized".to_string())?;
...
pub async fn copy_path(
    ...
    file_manager
        .copy_files(vec![PathBuf::from(source)], Path::new(destination))
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn move_path(
    ...
    use std::path::PathBuf;
    file_manager
        .move_files(vec![PathBuf::from(source)], Path::new(destination))
        .await
        .map_err(|e| e.to_string())?;
