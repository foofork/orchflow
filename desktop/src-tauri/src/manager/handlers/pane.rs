...
use crate::manager::{Manager, PaneType, ShellType, Event};
...
pub async fn create_pane(
    ...
    let pane_id = manager.mux_backend
    ...
    if let Some(cmd) = final_command {
        manager.mux_backend.send_keys(&pane_id, &cmd).await
            .map_err(|e| e.to_string())?;
        manager.mux_backend.send_keys(&pane_id, "\n").await
            .map_err(|e| e.to_string())?;
    }
    ...
    let pane_state = manager.state_manager.create_pane(
        session_id.clone(),
        state_pane_type,
        Some(pane_id.clone()),
...
pub async fn close_pane(manager: &Manager, pane_id: String) -> Result<Value, String> {
    ...
    manager.mux_backend.kill_pane(&pane_id).await
        .map_err(|e| e.to_string())?;
...
pub async fn resize_pane(
    ...
    // Resize in backend
    manager.mux_backend.resize_pane(pane_id, crate::mux_backend::PaneSize { width, height }).await
        .map_err(|e| e.to_string())?;
