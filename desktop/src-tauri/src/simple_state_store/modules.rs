...
use chrono::Utc;
use uuid::Uuid;
use super::types::{Module, CreateModule, UpdateModule};
...
impl ModuleRepository {
    ...
    pub async fn update_module(&self, module_id: &str, update_module: UpdateModule) -> SqliteResult<Option<Module>> {
        ...
        if let Some(enabled) = update_module.enabled {
            ...
            param_values.push(if enabled { "1".to_string() } else { "0".to_string() });
        }
    ...
    pub async fn is_module_enabled(&self, module_id: &str) -> SqliteResult<bool> {
        let conn = self.conn_manager.conn.lock().unwrap();
        let enabled: Option<bool> = conn.query_row(
    ...
    pub async fn update_module_version(&self, module_id: &str, version: &str) -> SqliteResult<bool> {
        let conn = self.conn_manager.conn.lock().unwrap();
        let rows_affected = conn.execute(
