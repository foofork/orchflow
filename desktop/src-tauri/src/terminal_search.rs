...
impl TerminalSearcher {
    ...
    pub async fn search_all_panes(
        ...
        let session = state_manager.get_session(session_id).await.ok_or_else(|| {
    ...
    pub async fn search_pane(
        ...
        let pane =
            state_manager
                .get_pane(pane_id)
                .await
                .ok_or_else(|| OrchflowError::PaneNotFound {
    ...
    pub fn highlight_matches(
        ...
        let re = Regex::new(&pattern).map_err(|e| OrchflowError::ValidationError {
            field: "query".to_string(),
            reason: format!("Invalid regex: {}", e),
        })?;
