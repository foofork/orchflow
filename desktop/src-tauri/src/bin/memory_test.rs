...
async fn main() {
    ...
    let after_store = system
        .processes()
        .get(&pid)
        .map(|p| p.memory() / 1024)
        .unwrap_or(0);
    println!(
        "After StateStore: {} KB (+{} KB)",
        after_store,
        after_store - baseline_memory
    );

    // 2. Mux Backend
    let _mux_backend = mux_backend::create_mux_backend();
    system.refresh_processes();
    let after_mux = system
        .processes()
        .get(&pid)
        .map(|p| p.memory() / 1024)
        .unwrap_or(0);
    println!(
        "After MuxBackend: {} KB (+{} KB)",
        after_mux,
        after_mux - after_store
    );
    ...
    system.refresh_processes();
    let after_manager = system
        .processes()
        .get(&pid)
        .map(|p| p.memory() / 1024)
        .unwrap_or(0);
    println!(
        "After Manager setup: {} KB (+{} KB)",
        after_manager,
        after_manager - after_mux
    );

    // 4. Plugin Registry
    let _plugin_registry = orchflow::plugins::PluginRegistry::new();
    system.refresh_processes();
    let after_plugins = system
        .processes()
        .get(&pid)
        .map(|p| p.memory() / 1024)
        .unwrap_or(0);
    println!(
        "After PluginRegistry: {} KB (+{} KB)",
        after_plugins,
        after_plugins - after_manager
    );

    // Summary
    println!("\n=== Memory Usage Summary ===");
    println!(
        "Total memory used: {} KB ({:.1} MB)",
        after_plugins,
        after_plugins as f64 / 1024.0
    );
    ...
    if after_plugins < 100 * 1024 {
        ...
        println!(
            "❌ Memory usage exceeds target by {:.1} MB",
            (after_plugins as f64 / 1024.0) - 100.0
        );
    }
