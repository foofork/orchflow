...
## Project Purpose
orchflow is a terminal-based IDE that combines Neovim's editing power with intelligent terminal orchestration. Think VS Code but terminal-first, faster, and with smart automation.

## Core Rules
**Quality:** Commits pass tests/types/lints · CI green · Never --no-verify  
**Scope:** Assigned work only · Match existing style · Confirm API changes  
**Process:** TDD always (test→fail→code→pass→refactor) · ≤500 LOC/file  
**Security:** Validate inputs · Handle errors · No secrets · Completion checks

## Workflow
**Before:** git status · test · clarify scope  
**During:** 1 intent = 1 commit · Test→Fail→Code→Pass→Refactor  
**Commit:** feat|fix(area): msg · Green CI · No TODOs  
**PR:** Next steps · Risks · Tech debt

## Documentation Rules
**Where to Document:**
- **Roadmap Updates** → [DEVELOPMENT_ROADMAP.md](DEVELOPMENT_ROADMAP.md)
- **Architecture Changes** → [docs/architecture/UNIFIED_ARCHITECTURE.md](docs/architecture/UNIFIED_ARCHITECTURE.md)
- **API Changes** → [docs/api/](docs/api/) or specific API docs
- **Test Strategy** → [docs/TEST_STRATEGY.md](docs/TEST_STRATEGY.md)

**NOT in Random Files** - No roadmap items in component files, no architecture decisions in test files

## Navigation
- **Development Hub**: [DEVELOPMENT.md](DEVELOPMENT.md)
