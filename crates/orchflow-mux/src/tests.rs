    ...
    async fn create_session(&self, name: &str) -> Result<String, MuxError> {
        if *self.fail_next.lock().await {
            *self.fail_next.lock().await = false;
            return Err(MuxError::SessionCreationFailed(format!(
...
mod backend_tests {
    ...
    async fn test_create_and_manage_panes() {
        ...
        // Create panes
        let _pane1 = backend
            .create_pane(&session_id, SplitType::Horizontal)
            .await
            .unwrap();
        let pane2 = backend
            .create_pane(&session_id, SplitType::Vertical)
            .await
            .unwrap();
    ...
    async fn test_multiple_sessions() {
        ...
        let session1 = backend.create_session("session1").await.unwrap();
        let session2 = backend.create_session("session2").await.unwrap();
        let _session3 = backend.create_session("session3").await.unwrap();

        // Verify all exist
        let sessions = backend.list_sessions().await.unwrap();
        assert_eq!(sessions.len(), 3);

        // Create panes in different sessions
        let pane1 = backend
            .create_pane(&session1, SplitType::None)
            .await
            .unwrap();
        let pane2 = backend
            .create_pane(&session2, SplitType::None)
            .await
            .unwrap();

        // Verify panes are in correct sessions
        let panes1 = backend.list_panes(&session1).await.unwrap();
        assert_eq!(panes1.len(), 1);
        assert_eq!(panes1[0].id, pane1);

        let panes2 = backend.list_panes(&session2).await.unwrap();
        assert_eq!(panes2.len(), 1);
        assert_eq!(panes2[0].id, pane2);

        // Kill one session shouldn't affect others
        backend.kill_session(&session1).await.unwrap();
        let sessions = backend.list_sessions().await.unwrap();
        assert_eq!(sessions.len(), 2);

        // Panes in session2 should still exist
        let panes2 = backend.list_panes(&session2).await.unwrap();
        assert_eq!(panes2.len(), 1);
    }

    #[tokio::test]
    async fn test_ui_event_types() {
        use crate::MuxUIEvent;
        use chrono::Utc;

        // Test serialization of UI events
        let output_event = MuxUIEvent::PaneOutput {
        ...
        assert!(json.contains("\"type\":\"PaneOutput\""));

        let exit_event = MuxUIEvent::PaneExit {
            pane_id: "test-pane".to_string(),
            exit_code: Some(0),
            timestamp: Utc::now(),
        };

        let json = serde_json::to_string(&exit_event).unwrap();
        assert!(json.contains("\"type\":\"PaneExit\""));
    }
}
