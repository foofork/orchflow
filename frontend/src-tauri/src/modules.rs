...
impl ModuleLoader {
    pub fn new(modules_dir: PathBuf, state_store: std::sync::Arc<crate::simple_state_store::SimpleStateStore>) -> Self {
        Self {
            modules_dir,
    ...
    async fn load_module_from_dir(&mut self, dir: &Path) -> Result<String, String> {
        ...
        let db_module = self.state_store
            .list_modules()
            .await
        ...
        let config = if let Some(db_module) = db_module {
            // Parse stored config
            serde_json::from_str(&db_module.manifest)
                .unwrap_or_else(|_| ModuleConfig {
                    enabled: true,
    ...
    pub async fn set_module_enabled(&mut self, name: &str, enabled: bool) -> Result<(), String> {
        if let Some(module) = self.loaded_modules.get_mut(name) {
            ...
            // Update in database
            let config_json = serde_json::to_string(&module.config)
                .map_err(|e| e.to_string())?;
