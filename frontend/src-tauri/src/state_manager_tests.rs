...
mod state_manager_tests {
    ...
    async fn test_pane_lifecycle() {
        ...
        // Create pane
        let pane = state_manager.create_pane(
            session.id.clone(), 
            PaneType::Terminal, 
        ...
        state_manager.update_pane(updated_pane.clone()).await.unwrap();
    ...
    async fn test_layout_management() {
        ...
        let (child1_id, child2_id) = updated_layout.split_pane(
            &root_id, 
            crate::layout::SplitType::Horizontal, 
            60
        ).unwrap();
        
        state_manager.update_layout(&session.id, updated_layout.clone()).await.unwrap();
    ...
    async fn test_multiple_sessions_and_panes() {
        ...
        let session2 = state_manager.create_session("session-2".to_string()).await.unwrap();
        
        // Create panes in each session
        let pane1 = state_manager.create_pane(
            session1.id.clone(), 
            PaneType::Terminal, 
            Some("backend-1".to_string())
        ).await.unwrap();
        
        let pane2 = state_manager.create_pane(
            session1.id.clone(), 
            PaneType::Editor, 
            Some("backend-2".to_string())
        ).await.unwrap();
        
        let pane3 = state_manager.create_pane(
            session2.id.clone(), 
            PaneType::Terminal, 
    ...
    async fn test_state_persistence() {
        ...
        // Create state manager and add some data
        {
            ...
            let session = state_manager.create_session("persistent-session".to_string()).await.unwrap();
            let pane = state_manager.create_pane(
                session.id.clone(), 
                PaneType::Terminal, 
    ...
    async fn test_event_emission() {
        ...
        let session = state_manager.create_session("event-test".to_string()).await.unwrap();
        ...
        match event {
            crate::state_manager::StateEvent::SessionCreated { session: event_session } => {
