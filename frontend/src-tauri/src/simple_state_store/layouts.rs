...
use chrono::Utc;
use uuid::Uuid;
use super::types::{Layout, CreateLayout, UpdateLayout};
...
impl LayoutRepository {
    ...
    pub async fn update_layout(&self, layout_id: &str, update_layout: UpdateLayout) -> SqliteResult<Option<Layout>> {
        ...
        if let Some(is_active) = update_layout.is_active {
            ...
            param_values.push(if is_active { "1".to_string() } else { "0".to_string() });
    ...
    pub async fn get_layout_by_name(&self, session_id: &str, name: &str) -> SqliteResult<Option<Layout>> {
        let conn = self.conn_manager.conn.lock().unwrap();
        conn.query_row(
            "SELECT id, session_id, name, layout_data, is_active, created_at, updated_at
