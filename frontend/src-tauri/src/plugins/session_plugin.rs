...
impl SessionPlugin {
    pub fn new() -> Self {
        ...
        templates.insert("dev".to_string(), SessionTemplate {
        ...
        templates.insert("test".to_string(), SessionTemplate {
    ...
    async fn create_from_template(
        ...
        if let Some(ctx) = &self.context {
            // Create session
            let session_result = ctx.execute(Action::CreateSession {
                name: session_name,
            }).await?;
    ...
    async fn handle_request(&mut self, method: &str, params: Value) -> Result<Value, String> {
        match method {
            "session.createFromTemplate" => {
                let template_name = params["template"].as_str()
                    .ok_or("Missing template name")?;
                let session_name = params["name"].as_str()
                    .unwrap_or("New Session")
                    .to_string();
                
                let session = self.create_from_template(template_name, session_name).await?;
                Ok(serde_json::to_value(session).unwrap())
            }
            
            "session.listTemplates" => {
                let templates: Vec<_> = self.templates.iter()
                    .map(|(id, template)| json!({
                        "id": id,
                        "name": template.name,
            ...
            "session.listSnapshots" => {
                ...
                let snapshots: Vec<_> = self.snapshots.iter()
                    .filter(|(id, _)| session_id.is_none() || session_id == Some(id.as_str()))
                    .map(|(id, snapshot)| json!({
                        "session_id": id,
            ...
            "session.setAutoSave" => {
                self.auto_save = params["enabled"].as_bool()
                    .unwrap_or(true);
                self.auto_save_interval_secs = params["interval_secs"].as_u64()
                    .unwrap_or(300);
            ...
            "session.importConfig" => {
                ...
                let snapshot: SessionSnapshot = serde_json::from_value(config)
