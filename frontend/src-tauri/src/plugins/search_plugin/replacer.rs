...
impl TextReplacer {
    ...
    pub async fn replace_in_file(
        ...
        // Perform replacement
        let (new_content, replacements_made) = self.perform_replacement(&content, &regex, &request.replacement);
    ...
    fn perform_replacement(&self, content: &str, regex: &regex::Regex, replacement: &str) -> (String, usize) {
        let mut count = 0;
    ...
    fn generate_preview(&self, old_content: &str, new_content: &str, replacements: usize) -> String {
        if replacements == 0 {
        ...
        preview.push(format!("--- Changes: {} replacements made ---", replacements));
