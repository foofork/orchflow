...
mod tests {
    ...
    use tokio::fs;
    use crate::project_search::{ProjectSearch, SearchOptions, SearchMatch, FileSearchResult, ReplaceResult};
...
"#
    ...
    async fn test_replace_dry_run() -> Result<()> {
        ...
        let total_replacements: usize = replace_results.iter()
    ...
    async fn test_replace_actual() -> Result<()> {
        ...
        // Actual replace
        let replace_results = search.replace_in_files(options, "replaced_foo", false).await?;
...
let z = 789;"#
        ...
        search.replace_in_files(options, "const VAR = NUM;", false).await?;
