...
impl TerminalStreamManager {
    ...
    pub async fn create_terminal(
        ...
        // Create PTY
        let pty_handle = self.pty_manager.create_pty(
            terminal_id.clone(),
            shell,
            rows,
            cols,
        ...
        self.terminals.write().await.insert(terminal_id.clone(), terminal_state);
    ...
    pub async fn get_process_info(
        ...
        if let Some(pty_handle) = self.pty_handles.read().await.get(terminal_id) {
            pty_handle.get_process_info().await
                .map_err(|e| crate::error::OrchflowError::TerminalError {
    ...
    pub async fn get_terminal_health(
        ...
        let state = self.get_terminal_state(terminal_id).await
            .ok_or_else(|| crate::error::OrchflowError::TerminalError {
    ...
    pub async fn restart_terminal(
        ...
        let terminal_state = self.get_terminal_state(terminal_id).await
            .ok_or_else(|| crate::error::OrchflowError::TerminalError {
