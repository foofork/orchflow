...
impl SettingsManager {
    ...
    async fn validate_setting(&self, key: &str, value: &str) -> Result<()> {
        if key.is_empty() {
            return Err(OrchflowError::validation_error("setting_key", "Setting key cannot be empty"));
        }
    ...
    pub async fn get_theme(&self) -> Result<String> {
        Ok(self.get_setting("ui.theme").await?.unwrap_or_else(|| "dark".to_string()))
    }
    
    pub async fn set_editor_font_size(&self, size: u32) -> Result<()> {
        self.set_setting("editor.font_size", &size.to_string()).await
    }
    
    pub async fn get_editor_font_size(&self) -> Result<u32> {
        let size_str = self.get_setting("editor.font_size").await?.unwrap_or_else(|| "14".to_string());
        size_str.parse().map_err(|_| OrchflowError::validation_error("font_size", "Invalid font size"))
    }
    ...
    pub async fn get_terminal_shell(&self) -> Result<String> {
        Ok(self.get_setting("terminal.default_shell").await?.unwrap_or_else(|| "bash".to_string()))
    }
    
    pub async fn set_auto_save(&self, enabled: bool) -> Result<()> {
        self.set_setting("editor.auto_save", &enabled.to_string()).await
    }
    
    pub async fn get_auto_save(&self) -> Result<bool> {
        let enabled_str = self.get_setting("editor.auto_save").await?.unwrap_or_else(|| "true".to_string());
        enabled_str.parse().map_err(|_| OrchflowError::validation_error("auto_save", "Invalid boolean value"))
    }
}